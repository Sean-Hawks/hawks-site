...
test("feeds include public content and only full Library reviews", () => {
  const items = buildFeedItems(
    [post, { ...post, status: "draft" }, { ...post, status: " PRIVATE " }],
    [{ id: "a", title: "Note", date: "2026-01-01", desc: "Body" }],
    [
      {
        slug: "a",
        category: "book",
      ...
      { slug: "b", category: "book", hasReview: false },
      {
        slug: "c",
        category: "book",
        hasReview: true,
  ...
  assert.deepEqual(
    Array.from(items, (item) => item.channel),
    ["blog", "talk", "library"],
  );
});
...
test("RSS preserves permalink GUIDs, escapes XML and identifies its channel", () => {
  ...
  assert.match(
    rss,
    /<guid isPermaLink="true">https:\/\/hawks.tw\/blog\/post\/<\/guid>/,
  );
  assert.match(
    rss,
    /href="https:\/\/hawks.tw\/feeds\/blog\/rss.xml" rel="self"/,
  );
...
test("empty channels and invalid dates produce valid structure without invalid dates", () => {
  ...
  const rss = renderFeed(
    buildFeedItems([{ ...post, date: "unknown" }], [], []),
  );
