...
test("malformed or poisoned browser data cannot inject links or crash the shelf", () => {
  for (const data of [
    ...
    assert.equal(parseReadingList(data).length, 0);
  const parsed = parseReadingList(
    raw([
      null,
      entry,
      entry,
      { ...entry, id: "javascript:alert(1)" },
      { ...entry, id: "talk:2026-09-19", savedAt: -1 },
      { ...entry, id: "post:other", href: "https://evil.example/" },
    ]),
  );
...
test("save, mark read, remove and undo preserve the original saved time without duplicates", () => {
  ...
  assert.equal(entries[0].savedAt, 100);
  assert.equal(
    applyReadingAction(entries, { type: "unread", id: entry.id })[0].readAt,
    null,
  );
});
test("the list limit does not evict existing saves, and removing still works when full", () => {
  ...
  assert.equal(
    applyReadingAction(entries, { type: "toggle", id: "post:overflow" }).length,
    200,
  );
  assert.equal(
    applyReadingAction(entries, { type: "toggle", id: "post:p-0" }).length,
    199,
  );
});
function store(storage) {
  return load("reading-store", {
...
test("mutations read the latest browser value so a save from another tab is retained", () => {
  ...
  assert.deepEqual(
    Array.from(parseReadingList(current), (entry) => entry.id),
    ["post:hipac", "talk:note"],
  );
});
test("blocked reads and failed writes retain a usable temporary list and disclose non-persistence", () => {
  for (const storage of [
    ...
    assert.ok(
      s.useReadingList().entries.some((entry) => entry.id === "talk:note"),
    );
    s.updateReadingList({ type: "read", id: "talk:note" });
    assert.ok(
      s.useReadingList().entries.find((entry) => entry.id === "talk:note")
        .readAt,
    );
  }
});
