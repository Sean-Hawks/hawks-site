...
const publicStatus = (value?: string) =>
  !["draft", "private"].includes(value?.trim().toLowerCase() ?? "");
export function buildFeedItems(
  ...
  return [
    ...posts
      .filter((post) => publicStatus(post.status))
      .map((post) => ({
        id: `blog-${post.slug}`,
        title: post.title,
    ...
    ...talks
      .filter((talk) => publicStatus(talk.status))
      .map((talk) => ({
        id: `talk-${talk.id}`,
        title: talk.title,
        ...
        channel: "talk" as const,
        category: "Talk",
      })),
    ...library
      .filter((item) => item.hasReview && publicStatus(item.statusVisibility))
      .map((item) => ({
        ...
        category: "Library Review",
      })),
  ];
}
const timestamp = (date: string) => Date.parse(date) || 0;
function rssDate(date: string) {
  return Number.isNaN(Date.parse(date)) ? "" : new Date(date).toUTCString();
}
...
export function selectFeedItems(items: FeedItem[], channel: FeedChannel) {
  return items
    ...
    .sort(
      (a, b) =>
        timestamp(b.date) - timestamp(a.date) || a.id.localeCompare(b.id),
    )
    .slice(0, 50);
}
