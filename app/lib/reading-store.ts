...
const initial: {
  ...
  persistent: boolean;
  ready: boolean;
} = { entries: [], persistent: true, ready: false };
...
function getSnapshot() {
  ...
  try {
    ...
    memoryOnly = true;
    snapshot = { ...snapshot, persistent: false, ready: true };
  }
  return snapshot;
}
...
export function updateReadingList(action: ReadingAction) {
  ...
  if (
    ...
    before.entries.length >= READING_LIMIT &&
    (action.type === "toggle" || action.type === "restore")
  )
    return { applied: false, message: "清單已達 200 篇，請先移除一些內容。" };
  const raw = JSON.stringify({ version: 1, entries });
  try {
    if (!memoryOnly) localStorage.setItem(key, raw);
  } catch {
    memoryOnly = true;
  }
  lastRaw = raw;
  ...
  return {
    applied: true,
    message: memoryOnly ? "瀏覽器無法儲存，這次變更只會暫存在目前頁面。" : "",
  };
}
